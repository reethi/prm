...
public function getGoalsGrid($request,$user_data)
	...
	{   	
	 ...
	 public function SaveVideos($data){		
	   // print_r($data);exit;		
		...
		$query = $this->db->get_where('upload_files', array(		
	    ...
	    $result = $this->db->insert('upload_files', $data1);
	     if(!$result){		
		   		return false;		
		   	}		
		   else{		
		...
		//echo 'else';exit;		
		$sql="update upload_files set file_type = '".$data['files']."', file_url = '".$data['url']."',file_title = '".$data['title']."' where file_type = '".$data['files']."'";		
  ...
  public function getVideos($where=array()){	
  		$this->db->select('*');
...
public function save_files($data){		
	   ...
	   // print_r($data);exit;		
	    ...
	    $result = $this->db->insert('upload_files', $total_data);	
	    if(!$result){		
		   		return false;		
		   	}		
		   else{		
		...
		//echo 'else';exit;		
		$sql="update upload_files set file_name = '".$data['files']."',file_type = '".$data['type']."',file_url = '".$url."',file_title = '".$title."' where file_name = '".$data['files']."'";		
	 	$result = $this->db->query($sql);		
	 	//echo $this->db->last_query();exit;		
	}		
		   if(!$result){		
  ...
  public function save_files_edit($data){		
	    //print_r($data);exit;		
		$url = $this->input->post('url');		
		$title = $this->input->post('title');		
		$push_users_date = $this->input->post('push_users_date');		
		...
		//echo $id;exit;
		$sql="update upload_files set file_name = '".$data['files']."',file_type = '".$data['type']."',file_url = '".$url."',file_title = '".$title."' where id = '".$id."'";		
	 	$result = $this->db->query($sql);		
	 	//echo $this->db->last_query();exit;
		   if(!$result){		
		   		return false;		
  ...
  public function deleteDocs($where){
  	$result=$this->db->delete('upload_files',$where);
  	if(!$result){		
		   		return false;		
		   	}		
		   else{		
  ...
  public function deleteVideos($where){
  	$result=$this->db->delete('upload_files',$where);
  	if(!$result){		
		   		return false;		
		   	}		
