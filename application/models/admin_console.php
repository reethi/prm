<?php
class Admin_console extends CI_model {
...
public function getadvices()
	...
	public function getveglogGrid($request,$user_data)
	{
	     $sql_details = array('user' => $this->db->username,'pass' => $this->db->password,'db'   => $this->db->database,'host' => $this->db->hostname);
		  ...
		  $columns = array(
					...
					array( 'db' => 'servings_count', 
					       'dt' => 3,
						   'formatter' => function( $d, $row ) {
										   return ucwords(strtolower($d));
										}
						  ),
					array( 'db' => 'veglog.created_date', 
					       'dt' => 4,
						   'formatter' => function( $d, $row ) {
										   return ucwords(strtolower($d));
										}
		    ...
		    $join="";
			$query_columns_array=array('user_id','name','item_title','item_description','stanford_code','veglog.veg_item_id','servings_count','veglog_date','user_session_id','veglog.created_date');
		 	
			$custom_where=array();
			$join.=" JOIN veg_logging veglog ON usr.id=veglog.user_id ";
			$join.=" JOIN veg_items vegitem ON veglog.veg_item_id=vegitem.id ";

			if($user_data['veg_item_id']!=0){
				$custom_where[]='veglog.veg_item_id="'.$user_data['veg_item_id'].'"';
			}
			if(isset($user_data['veglog_date'])){
				$custom_where[]='veglog.veglog_date="'.$user_data['veglog_date'].'"';
			}
			if(isset($user_data['user_id'])){
				$custom_where[]='veglog.user_id="'.$user_data['user_id'].'"';
			}
	...
	public function getveggieGrid($request)
	{
	     $sql_details = array('user' => $this->db->username,'pass' => $this->db->password,'db'   => $this->db->database,'host' => $this->db->hostname);
		  $request['searchcolumns']=array();
		  ...
		  $columns = array(
						...
						array( 'db' => 'rec_status' ,
							   ...
							   'formatter' => function( $d, $row ) {
												...
												$return_value='<span id="action_'.$row['id'].'"><button  '.$action_class.' onclick=vegGridstatus('.$row['id'].','.$row['rec_status'].'); >'.$action.'</button></span>';
											   return $return_value;
											}
							),
		    ...
		    $join="";
			$query_columns_array=array('id','item_title','item_description','rec_status');
		 	
			$custom_where=array();

		$request['custom_where']=(count($custom_where)>0)?implode(" AND ",array_unique($custom_where)):'';
        $query_columns=implode(",",array_unique($query_columns_array));
	    $sql_query='SELECT $query_columns from veg_items usr '.$join;
	    return datatable::simple( $request, $sql_details,$sql_query,$query_columns,$columns,false );
